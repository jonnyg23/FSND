    ...
    for location in venue_location:
        ...
        for venue in venue_query:
            if (venue.city == location[0]) and (venue.state == location[1]):
                # If venue needs to be added, count number of
...
def delete_venue(venue_id):
    ...
    if not venue2Delete:
        ...
        if error_deleting:
            ...
            flash(
                f'An error occurred while deleting venue {venue2Delete_name}.')
            abort(500)
        else:
            return jsonify({
                'deleted': True,
...
def create_artist_submission():
    ...
    if not form.validate():
        ...
        try:
            error_inserting = False
            artist_submission = Artist(name=name, city=city, state=state, phone=phone, genres=genres, image_link=image_link,
...
def delete_artist(artist_id):
    ...
    if not artist2Delete:
        ...
        if error_deleting:
            ...
            flash(
                f'An error occurred while deleting artist {artist2Delete_name}.')
            abort(500)
        else:
            return jsonify({
                'deleted': True,
